...
void BM(const Instance& inst, Solution& sol) {
	...
	vector<GRBLinExpr> startsHere(inst.W+1, 0); 
	vector<GRBLinExpr> endsHere(inst.W+1, 0); 
    vector<GRBLinExpr> height(inst.W + 1, 0);   
    for (int j = 0; j < inst.n; j++) { 	
		if(inst.items[j][2] == 0) continue; 
        for (int i = 0; i <= inst.W - inst.items[j][0]; i++) { 
			if(NPs[j][i]){
				...
				startsHere[i] += x[i][j];
				endsHere[i+inst.items[j][0]] += x[i][j];
			}
        }
    }
    model.update();
    ...
    for (int j = 0; j < inst.n; j++) { 	
		if(inst.items[j][2] == 0) continue; 
        for (int i = 1; i <= inst.W - inst.items[j][0]; i++) { 
			if(NPs[j][i]){
				model.addConstr(x[i][j] <= inst.items[j][2] * endsHere[i]);
			}
        }
    }
