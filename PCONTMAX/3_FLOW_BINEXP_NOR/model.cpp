...
void FLOW(const Instance& inst, Solution& sol) {
		...
		for (int i = 0; i <= inst.W - inst.items[j][0]; i++){ 	
			if(NPs[j][i]){		
				for (int k = 0; k < ceil(log2(inst.items[j][2]))+1; k++){
					arcs.push_back({i,i+inst.items[j][0],inst.items[j][1],j,pow(2,k)});
					isWActive[i] = true;
	...
	for (int i = 0; i < isWActiveV.size()-1; i++)
		arcs.push_back({isWActiveV[i],isWActiveV[i+1],1,-1,1});
    ...
    for (int k = 0; k < arcs.size(); k++){
		cIn[arcs[k][1]] += arcs[k][2] * x[k] * arcs[k][4];
        cOut[arcs[k][0]] += arcs[k][2] * x[k] * arcs[k][4];
		if(arcs[k][3] >= 0) assigned[arcs[k][3]] += x[k] * arcs[k][4];
    }
