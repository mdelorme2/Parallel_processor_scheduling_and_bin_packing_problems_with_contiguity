...
void CPM(const Instance& inst, Solution& sol) {
	...
	if(n2>0){
		IloOr OR_const0(env);
		for (int j = 0; j < n2; j++)
			OR_const0.add(IloStartOf(x[j]) == 0);
		model.add(OR_const0);
		
		for (int t = 1; t < inst.W; t++) { 
			IloOr OR_const1(env);
			IloOr OR_const2(env);
			for (int j = 0; j < n2; j++) {
				OR_const1.add(IloStartOf(x[j]) ==  t);
				OR_const2.add(IloEndOf(x[j]) ==  t);
			}
			model.add(IloIfThen(env,OR_const1 == true,OR_const2 == true));
		}
	}
