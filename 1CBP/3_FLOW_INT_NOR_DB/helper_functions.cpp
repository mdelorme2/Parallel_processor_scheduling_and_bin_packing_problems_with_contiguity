...
Instance readInstance(const string& filename, Solution& sol) {
	...
	for (int j = 0; j < inst.n; j++) {
		...
		if(sumAreaL + sumAreaR <= inst.W * heightL && heightL >= heightR){
			if (CP(inst2,heightL,sol.assignedBin,sol.LB) == 0){
	...
	inst.LUB = CPM(inst);
	inst.NPs.resize(inst.n,vector<bool>(inst.LUB,false));
	for (int j = 0; j < inst.n; j++) {
		if(inst.items[j][2] == 0) continue; 
		inst.NPs[j][0] = true;
		for (int jp = 0; jp<inst.n;jp++){
			...
			if (jp == j) lim--; 
			for(int l = 0; l<lim;l++){
				for (int i = inst.LUB-inst.items[jp][1] - 1;i >= 0; i--){
					if (inst.NPs[j][i]) inst.NPs[j][i+inst.items[jp][1]] = true;
				}
			}
		}
	}
	...
	inst.arcs = arcs;
	inst.RCs.resize(arcs.size());
    return inst;
}

int CP(const Instance& inst, const int& heightL, vector<vector<int> >& assignedBin, const int& LB) {
    ...
    for (int j = 0; j < n2; j++) { 
		...
		x[j].setStartMin(0);
		x[j].setStartMax(heightL - items2[j][1]);
	...
	if(cp.solve()){
        ...
        for (int j = 0; j < n2; j++){                                 
			...
			assignedBin[items2[j][2]].push_back(LB + cp.getStart(x[j]));
		}
		return 0;
	}
...
void printSInfo(const Solution& sol, const Instance& inst){
	vector<vector<int> > bins(sol.UB);
	vector<int>	load(sol.UB,0);
	for (int j = 0; j < sol.assignedBin.size();j++){
		for (int i = 0; i < sol.assignedBin[j].size();i++){
			for (int ip = sol.assignedBin[j][i]; ip < sol.assignedBin[j][i] + inst.items[j][1];ip++){
				load[ip] += inst.items[j][0];
				bins[ip].push_back(j);
			}
		}
	}
