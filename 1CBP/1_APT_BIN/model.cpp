...
void APT(const Instance& inst, Solution& sol) {
	...
	vector<vector<GRBLinExpr> > assignedQorAbove(LUB, vector<GRBLinExpr> (n2,0));
	vector<GRBLinExpr> lastrowQorBelow(LUB+1,0);
    ...
    for (int j = 0; j < n2; j++) { 	
        for (int i = 0; i <= LUB - items2[j][1]; i++){
			...
			for (int ip = i; ip <= LUB - items2[j][1]; ip++){
				assignedQorAbove[i][j] += x[ip][j];
			}
		}
    }   
    model.update();
	...
	for (int i = 0; i <= LUB; i++){
		...
		for(int ip = 0; ip <= i; ip++)
			lastrowQorBelow[i] += zvec[ip];
	}
	model.update();

    // create assignment constraints
    for (int j = 0; j < n2; j++){      										
		model.addConstr(assignedQorAbove[0][j] == 1); 						
	}
	...
	// create last row constraints 
    for (int j = 0; j < n2; j++) { 	
        for (int i = 0; i <= LUB - items2[j][1]; i++){	
			model.addConstr(assignedQorAbove[i][j] + lastrowQorBelow[i + items2[j][1] - 1] <= 1);
		}
	}
	model.addConstr(lastrowQorBelow[LUB] == 1);
