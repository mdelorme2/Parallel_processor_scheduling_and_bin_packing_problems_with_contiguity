...
int main(int argc, char **argv){          
	...
	Solution sol; sol.LB = 0; sol.UB = 0; sol.contR = 0; 
    ...
    // find the APT solution
	APTPLP(inst);
	int DB = max(LB(inst),int(ceil(inst.contR-EPSILON)));
	int TL = 3600 - (getCPUTime() - start);

	// Compute normal patterns for DB
	vector<bool> DBNP(inst.LUB,false);
	...
	while(TL >= 1 && APTP(inst,sol,DB,TL) == -1){
